function [T] = processing_dealias2_JY2(sp_loaded, bg_loaded, PSF1)
...
%             sp_loaded_norm(i,:,:,ii) = temp/mode(temp(:))-1;
        end
    end     
    ...
    for r = 0:2
        ...
        for pattern = 1:4
            u_eff(pattern,:,:,:) = exp(-1i*2*pi*r/3)*fftn(gpuArray(squeeze(U_modi(pattern,:,:,:))));  % exponential for global phase?
        end
        u_eff = gather(u_eff);
        clear U_modi
